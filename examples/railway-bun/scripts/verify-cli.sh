...
RAILWAY_ENVS=(development preview production)
...
for env_name in "${RAILWAY_ENVS[@]}"; do
...
for env_name in "${RAILWAY_ENVS[@]}"; do
  for key in PUBLIC_APP_NAME API_BASE_URL LOAD_ADD_ONLY_KEY LOAD_TEMP_KEY BETTER_ENV_UPSERT_KEY; do
    railway variable delete "$key" --environment "$env_name" --json >/dev/null 2>&1 || true
  done
done

cat > "${TEMP_ENV_FILES[0]}" <<'ENV'
PUBLIC_APP_NAME=Better Env Railway Bun Demo via Load Upsert
API_BASE_URL=https://api.railway.demo.internal/v1
LOAD_TEMP_KEY=temp-value
ENV

cat > "${TEMP_ENV_FILES[1]}" <<'ENV'
PUBLIC_APP_NAME=Better Env Railway Bun Demo via Load Update
API_BASE_URL=https://api.railway.demo.internal/v2
...
PUBLIC_APP_NAME=Better Env Railway Bun Demo via Load Replace
API_BASE_URL=https://api.railway.demo.internal/v3
ENV
...
run "$BE_BIN" add PUBLIC_APP_NAME "Better Env Railway Bun Demo Initial" --environment development
run "$BE_BIN" update PUBLIC_APP_NAME "Better Env Railway Bun Demo Updated" --environment development
...
run railway variable list --environment development --json
...
if ! rg -q '^PUBLIC_APP_NAME=Better Env Railway Bun Demo via Load Replace$' ".env.development"; then
