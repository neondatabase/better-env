...
const fakeWranglerBin = path.join(packageRoot, "test", "bin", "wrangler");

beforeAll(async () => {
  await fs.promises.chmod(fakeWranglerBin, 0o755);
});

describe("better-env runtime with Cloudflare adapter (e2e)", () => {
  ...
  it("load --replace removes keys not present in source file", async () => {
    ...
    const env = store.env.production ?? {};

    expect(env.A).toBeUndefined();
    expect(env.B).toBe("22");
    expect(env.C).toBe("3");
  });

  it("pull fails because Cloudflare does not expose secret values", async () => {
    const projectDir = await makeTempProject();
    await writeConfig(projectDir);
...
async function writeConfig(projectDir: string): Promise<void> {
  ...
  const config = [
    `import { cloudflareAdapter, defineBetterEnv } from ${JSON.stringify(pkgIndexUrl)};`,
    "",
    "export default defineBetterEnv({",
    `  adapter: cloudflareAdapter({ wranglerBin: ${JSON.stringify(fakeWranglerBin)} }),`,
    "});",
    "",
  ].join("\n");
...
async function readStore(projectDir: string): Promise<Store> {
  const p = path.join(projectDir, ".better-env-test", "wrangler-store.json");
  const raw = await fs.promises.readFile(p, "utf8");
  const parsed = JSON.parse(raw);
  if (!isStore(parsed)) {
    throw new Error("Invalid fake wrangler store format.");
  }
  return parsed;
}
