...
const fakeConvexBin = path.join(packageRoot, "test", "bin", "convex");

beforeAll(async () => {
  await fs.promises.chmod(fakeConvexBin, 0o755);
});

describe("better-env runtime with Convex adapter (e2e)", () => {
  it("init succeeds, upserts, and pulls development env", async () => {
    ...
    const pull = await runCli(projectDir, [
      "pull",
      "--environment",
      "development",
    ]);
    ...
    const envDev = await fs.promises.readFile(
      path.join(projectDir, ".env.development"),
      "utf8",
    );
    expect(envDev).toContain("API_URL=https://example.com");
  });
  ...
  it("load --replace deletes keys not present in file", async () => {
    ...
    const envProd = await fs.promises.readFile(
      path.join(projectDir, ".env.production"),
      "utf8",
    );
    expect(envProd).not.toContain("A=1");
    expect(envProd).toContain("B=22");
    expect(envProd).toContain("C=3");
  });
});
...
async function writeConfig(projectDir: string): Promise<void> {
  ...
  const config = [
    `import { convexAdapter, defineBetterEnv } from ${JSON.stringify(pkgIndexUrl)};`,
    "",
    "export default defineBetterEnv({",
    `  adapter: convexAdapter({ convexBin: ${JSON.stringify(fakeConvexBin)} }),`,
    "});",
    "",
  ].join("\n");
