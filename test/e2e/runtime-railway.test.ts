...
const fakeRailwayBin = path.join(packageRoot, "test", "bin", "railway");

beforeAll(async () => {
  await fs.promises.chmod(fakeRailwayBin, 0o755);
});

describe("better-env runtime with Railway adapter (e2e)", () => {
  it("init links project, upserts, and pulls preview env", async () => {
...
async function writeConfig(projectDir: string): Promise<void> {
  ...
  const config = [
    `import { defineBetterEnv, railwayAdapter } from ${JSON.stringify(pkgIndexUrl)};`,
    "",
    "export default defineBetterEnv({",
    `  adapter: railwayAdapter({ railwayBin: ${JSON.stringify(fakeRailwayBin)} }),`,
    "});",
    "",
  ].join("\n");
